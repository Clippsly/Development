...
function loadCliptivityPage(page) {
    ...
    fetch(`https://api.clippsly.app/endpoints/functions/cliptivity?page=${page}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            if (Array.isArray(data.actionList) && data.actionList.length > 0) {
                data.actionList.forEach(action => {
                    ...
                    const currentTier = action.userInfo.membershipTier ?? 0;
                    const actionClass = tierClasses[currentTier] ?? '';
                    ...
                    cliptivityDiv.innerHTML = `
                        ...
                        <div class="cliptivity-content">
                            <p class="cliptivity-author">
                                ...
                                <span class="cliptivity-action ${actionClass}">${actionDescription}</span>
                            </p>
                            <p class="cliptivity-date"t>${formatTimestamp(action.actionTime)}</p>
                        </div>
                    `;
